...
- (void)testDatabasePageRangeResponseDecoding
{
    ...
    ResponseHeader *header = [_decoder decodeHeader:headerData];
    XCTAssertNotNil(header);

    NSData *responseData = [data subdataWithRange:NSMakeRange(4, [data length] - 4)];

    ReceiverResponse *response = [_decoder decodeResponse:responseData toRequest:[[ReadDatabasePageRangeRequest alloc] initWithRecordType:UserEventData] withHeader:header];
...
{       
    ...
    ReceiverResponse *response = [_decoder decodeResponse:responseData toRequest:[[ReadDatabasePagesRequest alloc] initWithRecordType:EGVData pageNumber:1 numberOfPages:2] withHeader:header];
    XCTAssertNotNil(response, @"response should not be nil for input [%s]", [input UTF8String]);
    XCTAssertNotNil(response.payload, @"response.payload should not be nil for input [%s]", [input UTF8String]);
...
-(void)testDatabasePageOfUserEventDecoding
{
    ...
    ReceiverResponse *response = [_decoder decodeResponse:responseData toRequest:[[ReadDatabasePagesRequest alloc] initWithRecordType:UserEventData pageNumber:1 numberOfPages:2] withHeader:header];
    XCTAssertNotNil(response, @"response should not be nil for input [%s]", [input UTF8String]);
    XCTAssertNotNil(response.payload, @"response.payload should not be nil for input [%s]", [input UTF8String]);
    RecordData *recordData = (RecordData *) response.payload;
